    ...
    <section class="flex flex-col flex-grow md:justify-center items-center">
      <div v-if="!resultsFound" class="max-w-md w-full p-6 bg-white rounded-lg shadow-md my-10">
        ...
        <form class="space-y-6" @submit.prevent="handleSubmit">
          <div>
            ...
            <input class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:shadow-outline" :class="{ 'border-red-500 shadow-outline-red focus:outline-none': firstSubmmited && !isSubmitting && error === notFoundMsg }"  id="nif" type="text" v-model="nif" placeholder="X00000000X">
          </div>
          <div>
            <label class="block text-gray-700 font-bold mb-2" for="email">
              Correo electrónico
            </label>
            <input class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:shadow-outline" :class="{ 'border-red-500 shadow-outline-red focus:outline-none': firstSubmmited && !isSubmitting && error === notFoundMsg }"  id="email" type="email" v-model="email" placeholder="ejemplo@correo.com">
          </div>
          <div>
        ...
        <p v-if="firstSubmmited && !isSubmitting" class="text-red-500 text-center mt-4">{{ error ? error : notFoundMsg }}</p>
      </div>
      <div v-else class="max-w-md w-full p-4 bg-white rounded-lg shadow-md my-10">
        ...
        <div v-if="activeTab === 'asistencia'">
          ...
          <button @click="download" v-if="results.asistencia >= 80.00" class="my-4 block mx-auto bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
        ...
        <div v-else-if="activeTab === 'microcursos'">
          <div v-if="!results.microcursos.doble && (results.microcursos.micro1 || results.microcursos.micro2)">
            <p class="font-bold text-gray-700 mb-2 text-center">{{ results.microcursos.micro1 ? results.microcursos.micro1 : results.microcursos.micro2 }}</p>
            <button @click="download" class="my-4 block mx-auto bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
              Descargar certificado
            </button>
          </div>
          <div v-else-if="results.microcursos.doble && results.microcursos.micro1 && results.microcursos.micro2">
            <p class="font-bold text-gray-700 mb-2 text-center">Has asistido a dos microcursos de un día</p>
            <div class="flex items-center justify-center">
              <button @click="download" class="mx-2 my-4 bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline">
...
import { IconArrowNarrowLeft, IconReload } from '@tabler/icons-vue';
...
const notFoundMsg = 'No se han encontrado resultados para el NIF y correo electrónico proporcionados.'
const results = ref<SearchResult>({
  id: null,
  asistencia: 20.017684,
  microcursos: {
      doble: false,
      micro1: 'Cositas chulas',
      micro2: 'Cositas más chulas',
    },
  poster: true,
});
const nif = ref('');
...
async function handleSubmit() {
  ...
  try {
    ...
    console.log(e)
    error.value = 'Ha ocurrido un error al intentar conseguir los certificados.'
  } finally {
...
async function download() {
  const certType = activeTab.value === 'microcursos' ? 'microcurso' : activeTab.value;
  try {
    const response = await fetch(`https://ceebi.wupp.dev/api/ceebi-ii/certificado/${certType}/?nif=${nif}&email=${email}`)
  } catch (e) {
    console.log(e)
    error.value = 'Ha ocurrido un error al intentar conseguir los certificados.'
  }
