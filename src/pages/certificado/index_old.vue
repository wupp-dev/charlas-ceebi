    ...
    <main class="flex flex-col flex-grow items-center md:justify-center">
      <div v-if="!resultsFound" class="max-w-md w-full p-6 bg-white rounded-lg shadow-md my-10">
        ...
        <form class="space-y-6" @submit.prevent="handleSubmit">
          <div>
            ...
            <input
              class="appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:shadow-outline"
              :class="{
      ...
      <div v-else class="max-w-md w-full p-4 bg-white rounded-lg shadow-md my-10">
        ...
        <div class="flex justify-between mb-4 border-b border-t">
          <button
            class="mx-2 text-lg font-bold focus:outline-none hover:text-blue-500"
            :class="{ 'text-blue-500': activeTab === 'asistencia' }"
          ...
          <button
            class="mx-2 text-lg font-bold focus:outline-none hover:text-blue-500"
            :class="{ 'text-blue-500': activeTab === 'microcursos' }"
          ...
          <button
            class="mx-2 text-lg font-bold focus:outline-none hover:text-blue-500"
            :class="{ 'text-blue-500': activeTab === 'poster' }"
        ...
        <div v-if="activeTab === 'asistencia'">
          ...
          ></p>
          <p class="font-bold text-zinc-900 mb-2 text-center">
          ...
          <button
            ...
            class="my-4 block mx-auto bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
          >
        ...
        <div v-else-if="activeTab === 'microcursos'">
          ...
          <div
            v-else-if="
              results.microcursos.doble && results.microcursos.micro1 && results.microcursos.micro2
            "
          >
            <div class="flex flex-col items-center justify-center">
              ...
              <button
                ...
                :class="test ? '!bg-gray-500 hover:bg-current' : ''"
                @click="download('microcurso', results.microcursos.micro1)"
                class="mx-2 my-4 bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
              >
        ...
        <div v-else-if="activeTab === 'poster'">
          ...
          <div v-if="results.poster">
            ...
            <button
              ...
              class="my-4 block mx-auto bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded focus:outline-none focus:shadow-outline"
            >
              Descargar certificados (ZIP)
            </button>
          </div>
          <p v-else="!results.poster" class="font-bold text-red-500 text-center">
            No has presentado ningún póster, si crees que se trata de un error, conctacta con
            nosotros por correo <a href="mailto:info@biociencias.es">info@biociencias.es</a>
          </p>
        </div>
      </div>
    </main>
...
async function download(certType: string, micro?: string) {
  ...
  if (activeTab.value === 'asistencia') {
    ...
    error.value.poster = ''
    try {
      const url = `https://ceebi.wupp.dev/api/ceebi-ii/certificado/${path}/${results.value.id}.zip`
      const response = await fetch(url)
      if (response.status === 200) {
        window.open(url, '_blank')
      } else if (response.status === 404) {
